...
public class AggregatorMapClass extends Mapper<Text, Text, Text, WordStatisticsWritable> {
	...
	public void map(Text key, Text value, Context context)
		...
		//Replace commas in line with spaces for tokenizer
		line = line.replace(",", " ");
		...
		while (tokenizer.hasMoreTokens()) {
			...
			//Use switch statement against the value index
			//to determine which value we are currently saving
			
			String token = tokenizer.nextToken();
			switch(value_index){
				case 0:		document_count = Long.parseLong(token);
							break;
				case 1:		frequency = Long.parseLong(token);
							break;
				case 2:		frequency_squared = Long.parseLong(token);
							break;
				case 3:		mean = Double.parseDouble(token);
							break;
				case 4:		variance = Double.parseDouble(token);
							break;
				default:	break;
			}
			value_index++;
